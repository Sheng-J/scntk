    ...
    for i_seed in range(args.n_seeds):
        ...
        for i_test in range(args.n_tests):
            ...
            if args.exp_type == "complexity":
                ...
                assert nx == ny
                # TODO timeit
                if args.method == "mmd_o":
                    Kx = compute_gaussian_K(S_P, S_P, batch_size)
                    Ky = compute_gaussian_K(S_Q, S_Q, batch_size)
                    Kxy = compute_gaussian_K(S_P, S_Q, batch_size)
                    ...
                    print()
                elif args.method == "mmd_d":
                    Kx, model = compute_deep_K(S_P, S_P, batch_size, model)
                    Ky, _ = compute_deep_K(S_Q, S_Q, batch_size, model)
                    Kxy, _ = compute_deep_K(S_P, S_Q, batch_size, model)
                    sum_xx = (onp.sum(Kx) - onp.sum(onp.diag(Kx)) )/(nx*(nx-1))
                    sum_yy = (onp.sum(Ky) - onp.sum(onp.diag(Ky)) )/(ny*(ny-1))
                    sum_xy = (onp.sum(Kxy) - onp.sum(onp.diag(Kxy)) )/(nx*(ny-1))
                    mmd_slow = sum_xx - 2*sum_xy + sum_yy
...
# TODO type 1 vs type 2 error, just cifar10 vs mnist
